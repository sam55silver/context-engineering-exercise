...
1. Create a new file in `backend/migrations/` with format `NNN_description.py`:
...
2. Add it to the `MIGRATIONS` list in `migrate.py`:

```python
from migrations.002_add_user_preferences import AddUserPreferences

MIGRATIONS = [
    CreateTables(),
    AddUserPreferences(),  # Added
]
```
...
1. Create a new file in `backend/seeds/` with format `seed_*.py`:
...
2. Add it to the `SEEDERS` list in `seed.py`:

```python
from seeds.seed_users import SeedUsers
