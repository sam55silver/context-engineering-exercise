...
def get_watchlist(page: int = 1, size: int = 5):
    conn = get_conn()
    try:
        offset = (page - 1) * size  # Pagination bug fixed here
        rows = conn.execute(
            """
...
def add_to_watchlist(body: AddToWatchlist):
    ...
    try:
        ...
        print("Add to watchlist DB error:", e)
...
def get_recent():
    ...
    try:
        ...
        print("Recent watchlist DB error:", e)
        raise HTTPException(status_code=500, detail="Database error. Please try again.")
    finally:
        conn.close()
